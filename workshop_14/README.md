...
У режимі Lossless (без втрат якості) WebP перевершив PNG для графічного контенту, забезпечивши економію ваги файлу при збереженні ідеальної чіткості, що робить його найкращим вибором для скріншотів і логотипів. У режимі Lossy (з втратою якості) для фотографій формати WebP та AVIF показали значно кращу ефективність, ніж MozJPEG. 

## **ВИСНОВОК:**
Отже WebP є найбільш ефективним форматом: він перевершує PNG у lossless-стисненні для графіки та MozJPEG у lossy-стисненні для фотографій. Зміна розміру є найпотужнішим інструментом оптимізації, оскільки вона забезпечує найбільше зменшення ваги файлу.
